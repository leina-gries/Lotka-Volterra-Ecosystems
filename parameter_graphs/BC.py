...
def main():
    ...
    NW_Previous = N0W
    Prey_Var_Range = 600
    Predator_Var_Range = 400
    print('#from variables:', 'dt=', dt, 'NOB=', N0B, 'NOW=', N0W, 'Prey Growth Rate=',  Prey_Growth_Rate, 'Predator Death Rate=',  Predator_Death_Rate, 'NGen =', NGen, 'Prey Death Rate Range=', Prey_Var_Range, 'Predator Growth Rate=', Predator_Var_Range)
   ...
   # color_list = []
    for i in range(Prey_Var_Range):
       ...
       # divergex_list = []
       # divergey_list = []
        Prey_Side_Interaction_Rate = ((i+1)/10)
        for n in range(Predator_Var_Range):
            ...
            #3divergex_list = []
            #3ivergey_list = []
            Predator_Side_Interaction_Rate = ((n+1)/40)
            for z in range(NGen):
                ...
                if NB_Previous <= 0:
                    NB_New = 0
                    x1_data_list.append((Prey_Side_Interaction_Rate))
                ...
                if NW_Previous <= 0:
                    NW_New = 0
                    x2_data_list.append((Prey_Side_Interaction_Rate))
                ...
                if len(peak_list) >= 3:
                    if peak_list[1] > peak_list[0] and peak_list[2] > peak_list[1]:
                        divergex_list.append((Prey_Side_Interaction_Rate))
                        divergey_list.append((Predator_Side_Interaction_Rate))
          
                if z+1 == NGen:
                    x3_data_list.append((Prey_Side_Interaction_Rate))
